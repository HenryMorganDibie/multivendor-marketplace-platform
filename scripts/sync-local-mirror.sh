...
# the platform-backend, the platform-website, and the platform-vendor-portal into this
...
# ../the platform-backend, ../the platform-website, ../the platform-vendor-portal — adjust
# the paths below if yours live elsewhere.
set -euo pipefail

SANDBOX_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BACKEND_SRC="${1:-$SANDBOX_DIR/../the platform-backend}"
WEBSITE_SRC="${2:-$SANDBOX_DIR/../the platform-website}"
VENDOR_PORTAL_SRC="${3:-$SANDBOX_DIR/../the platform-vendor-portal}"

cd "$SANDBOX_DIR"

echo "Syncing the platform-backend from $BACKEND_SRC ..."
...
echo "Syncing the platform-website from $WEBSITE_SRC ..."
rm -rf website && mkdir website
(cd "$WEBSITE_SRC" && git archive HEAD) | tar -x -C "$SANDBOX_DIR/website"

echo "Syncing the platform-vendor-portal from $VENDOR_PORTAL_SRC ..."
