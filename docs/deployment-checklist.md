...
| `STRIPE_CHECKOUT_SUCCESS_URL` / `STRIPE_CHECKOUT_CANCEL_URL` | `createStripeCheckout` | Falls back to a placeholder `https://theplatform.com/...` URL | **Yes** — set these to the real frontend routes the vendor should land on after paying/cancelling, before going live with Stripe specifically. |
